...
import { Loader2, RefreshCw, Plus, Eye, EyeOff, Trash2, ExternalLink } from "lucide-react";

const DAYS = ["monday","tuesday","wednesday","thursday","friday","saturday","sunday"] as const;
const DAY_SHORT: Record<string, string> = { monday: "Mon", tuesday: "Tue", wednesday: "Wed", thursday: "Thu", friday: "Fri", saturday: "Sat", sunday: "Sun" };
const PLATFORM_LABELS: Record<string, string> = { instagram: "Instagram", x: "X / Twitter", twitter: "X / Twitter", linkedin: "LinkedIn", facebook: "Facebook", threads: "Threads", tiktok: "TikTok", youtube: "YouTube", bluesky: "Bluesky" };
...
export default function TenantDistribution() {
  ...
  const saveMut = trpc.licenseSettings.setBulk.useMutation({ onSuccess: () => toast.success("Settings saved") });
  ...
  const syncAccountsMut = trpc.distribution.syncBlotatoAccounts.useMutation({
    onSuccess: (r: any) => { toast.success("Synced " + r.count + " account(s)"); utils.distribution.getBlotatoAccounts.invalidate(); },
  ...
  const handleSaveSettings = async () => { if (licenseId) await saveMut.mutateAsync({ licenseId, settings: s }); };

  const addSlot = (platform: string) => setSlots(prev => [...prev, { platform, day: "monday", hour: 9, minute: 0 }]);
  const removeSlot = (index: number) => setSlots(prev => prev.filter((_, i) => i !== index));
  const updateSlot = (index: number, field: string, value: any) => setSlots(prev => prev.map((sl, i) => i === index ? { ...sl, [field]: value } : sl));
  ...
  const Toggle = ({ k, label, desc }: { k: string; label: string; desc?: string }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0" }}>
      ...
      <button onClick={() => update(k, s[k] === "true" ? "false" : "true")}
        style={{ width: 40, height: 22, borderRadius: 11, border: "none", cursor: "pointer", background: s[k] === "true" ? "#2dd4bf" : "#d1d5db", position: "relative", flexShrink: 0 }}>
  ...
  return (
    <TenantLayout pageTitle="Distribution" pageSubtitle="Social media distribution via Blotato" section="Social Media" saveAction={handleSaveSettings}>

      {/* Section 1 — Blotato Connection */}
      <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 2 }}>Blotato Connection</h3>
            <p style={{ fontSize: 12, color: "#6b7280", margin: 0 }}>
              {hasApiKey && accounts.length > 0
                ? "Connected — " + accounts.length + " platform" + (accounts.length !== 1 ? "s" : "")
                : hasApiKey ? "API key set — no platforms synced yet" : "Not connected"}
            </p>
          </div>
          <a href="https://my.blotato.com/settings" target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "#6b7280", textDecoration: "none" }}>
            my.blotato.com <ExternalLink size={10} />
          </a>
        </div>
        ...
        {hasApiKey && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "#22c55e" }}>API key configured</span>
            <button onClick={() => syncAccountsMut.mutate()} disabled={syncAccountsMut.isPending}
              style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 4, border: "1px solid #e5e7eb", background: "#fff", fontSize: 11, cursor: "pointer", color: "#374151" }}>
              {syncAccountsMut.isPending ? <Loader2 size={10} className="animate-spin" /> : <RefreshCw size={10} />}
              Sync Accounts
            </button>
          </div>
        )}
      ...
      {hasApiKey && (
        <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 12 }}>Connected Platforms</h3>
          {accounts.length === 0 ? (
            <p style={{ fontSize: 13, color: "#9ca3af" }}>
              No accounts found. Connect platforms at <a href="https://my.blotato.com/settings" target="_blank" rel="noopener noreferrer" style={{ color: "#2dd4bf" }}>my.blotato.com</a>, then click Sync Accounts above.
            </p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {accounts.map((a: any) => (
                <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 6, border: "1px solid #e5e7eb" }}>
                  <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, background: "#f3f4f6", color: "#374151", minWidth: 70, textAlign: "center" }}>
                    {PLATFORM_LABELS[a.platform] || a.platform}
                  </span>
                  <span style={{ fontSize: 13, color: "#374151", flex: 1 }}>@{a.username}</span>
                  <Toggle k={"blotato_" + a.platform + "_enabled"} label="" />
                  <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: "#d1fae5", color: "#065f46" }}>Connected</span>
                </div>
              ))}
            </div>
          )}
      ...
      {accounts.length > 0 && (
        <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 16 }}>Posts distribute to your next available time slot. If no slots are configured, posts go live immediately.</p>
          ...
          {accounts.map((account: any) => (
            <div key={account.platform} style={{ marginBottom: 16, padding: 16, borderRadius: 8, border: "1px solid #f3f4f6", background: "#fafafa" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                <span style={{ fontSize: 14, fontWeight: 600 }}>{PLATFORM_LABELS[account.platform] || account.platform}</span>
                <span style={{ fontSize: 12, color: "#6b7280" }}>@{account.username}</span>
              </div>
              {slots.filter(sl => sl.platform === account.platform).length === 0 && (
                <p style={{ fontSize: 12, color: "#9ca3af", marginBottom: 8 }}>No time slots — posts will go live immediately</p>
              )}
              {slots.map((slot, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                    ...
                    <button onClick={() => removeSlot(i)} style={{ background: "none", border: "none", cursor: "pointer", color: "#ef4444", padding: 4 }}><Trash2 size={14} /></button>
                  </div>
                );
              })}
      ...
      <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", marginBottom: 16 }}>
        ...
        {activityQuery.isLoading ? (
          ...
          <p style={{ color: "#9ca3af", fontSize: 13 }}>No distribution activity yet. Published articles will appear here once distributed.</p>
        ) : (
          <div>
            {(activityQuery.data?.items || []).map((item: any) => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid #f3f4f6" }}>
                <span style={{ padding: "2px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: "#f3f4f6", color: "#374151", width: 70, textAlign: "center", flexShrink: 0 }}>
                ...
                <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 10, fontWeight: 600, flexShrink: 0,
                ...
                {item.postUrl && (
                  <a href={item.postUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: "#2dd4bf", textDecoration: "none", flexShrink: 0 }}>View</a>
                )}
              </div>
            ))}
          </div>
      ...
      {/* Section 5 — Distribution Settings */}
      <div style={{ background: "#fff", borderRadius: 8, padding: 20, border: "1px solid #e5e7eb", marginBottom: 16 }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 12 }}>Distribution Settings</h3>
        <Toggle k="blotato_auto_distribute" label="Distribute on publish" desc="Automatically queue social posts when articles are published" />
        <div style={{ borderTop: "1px solid #f3f4f6", marginTop: 4 }} />
        <Toggle k="blotato_include_image" label="Include featured image" desc="Attach the article's featured image to social posts" />
        <Toggle k="blotato_include_excerpt" label="Include article excerpt" desc="Add the subheadline as post body text" />
      </div>
