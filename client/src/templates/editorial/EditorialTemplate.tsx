...
function EmptyState({ licenseSettings }: Pick<TemplateProps, "licenseSettings">) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "80px 24px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <h2 style={{ fontFamily: "var(--brand-font-heading, Georgia, serif)", fontSize: "clamp(28px, 4vw, 40px)", fontWeight: 700, color: "var(--brand-text-primary)", marginBottom: 12 }}>
        We're just getting started
      </h2>
      <p style={{ color: "var(--brand-text-secondary)", fontSize: 15, lineHeight: 1.6, marginBottom: 32 }}>
        ...
        <form onSubmit={e => { e.preventDefault(); if (email.trim() && consent) mutation.mutate({ email: email.trim() }); }}
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" required
              style={{ flex: 1, padding: "10px 14px", borderRadius: 6, border: "1px solid var(--brand-border)", fontSize: 14, background: "var(--brand-background)", color: "var(--brand-text-primary)" }} />
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", margin: "8px 0 0 0", fontSize: 12, color: "var(--brand-text-secondary)" }}>
            <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)} style={{ width: 14, height: 14, minWidth: 14, cursor: "pointer", accentColor: "var(--brand-primary)", margin: 0 }} />
            <span>I agree to receive the newsletter. <a href="/privacy" style={{ color: "var(--brand-link)", textDecoration: "underline" }}>Privacy Policy</a>.</span>
          </label>
        </form>
      )}
    </div>
...
function EditorialHomepage({ licenseSettings, articles, categories, mostRead }: TemplateProps) {
  ...
  const gridArticles = articles.slice(4, 16);
      ...
      {articles.length === 0 ? (
        ...
        <>
          ...
          <section id="main-content" style={{ maxWidth: 1280, margin: "0 auto", padding: 24 }}>
            <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "65% 35%", gap: 24 }}>
              <div>
          ...
          <AdZone placement="article" licenseSettings={licenseSettings} className="py-6" />

          {/* Main Content */}
          <div className="main-layout" style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px 48px", display: "grid", gridTemplateColumns: "1fr 320px", gap: 40 }}>
            <div>
              {/* Latest Grid */}
                    ...
                    {mostReadArticles.slice(0, 5).map((a, i) => (
                      <li key={a.id}>
                        ...
                        <a href={`/article/${a.slug}`} style={{ display: "flex", alignItems: "flex-start", gap: 16, textDecoration: "none", color: "inherit", padding: "4px 0" }}>
                          ...
                          <div>
                            <h3 style={{ fontSize: 15, fontFamily: "var(--brand-font-heading, Georgia, serif)", fontWeight: 600, color: "var(--brand-text-primary)", lineHeight: 1.3, margin: 0 }}>{a.headline}</h3>
            ...
            <aside style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div style={{ background: "var(--brand-surface)", padding: 20, borderRadius: 8, border: "1px solid var(--brand-border)" }}>
                <h3 style={{ fontFamily: "var(--brand-font-heading, Georgia, serif)", fontSize: 16, fontWeight: 700, marginBottom: 8, color: "var(--brand-text-primary)" }}>Newsletter</h3>
                <p style={{ fontSize: 13, color: "var(--brand-text-secondary)", marginBottom: 12 }}>Get updates from {licenseSettings.brand_site_name || "us"}</p>
...
export function EditorialTemplate(props: TemplateProps) {
  ...
  return (
    <div style={wrapperStyle}>
      {page === "home" && <EditorialHomepage {...props} />}
      ...
      {page === "search" && <EditorialHomepage {...props} />}
    </div>
  );
}
