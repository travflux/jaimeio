...
export const appRouter = router({
  ...
  templates: router({
    ...
    update: tenantOrAdminProcedure.input(z.object({
      ...
      if (!dbConn) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const { sql: sqlFn } = await import("drizzle-orm");
      await dbConn.execute(sqlFn`UPDATE article_templates SET name = COALESCE(${input.name}, name), description = COALESCE(${input.description}, description), promptTemplate = COALESCE(${input.promptTemplate}, promptTemplate), headline_format = COALESCE(${input.headlineFormat}, headline_format), tone = COALESCE(${input.tone}, tone), target_word_count = COALESCE(${input.targetWordCount}, target_word_count), schedule_frequency = COALESCE(${input.scheduleFrequency}, schedule_frequency), schedule_color = COALESCE(${input.scheduleColor}, schedule_color), is_active = COALESCE(${input.isActive !== undefined ? (input.isActive ? 1 : 0) : null}, is_active), article_format_type = COALESCE(${input.articleFormatType ?? null}, article_format_type), sentence_rhythm = COALESCE(${input.sentenceRhythm ?? null}, sentence_rhythm) WHERE id = ${input.id} AND (license_id = ${input.licenseId} OR licenseId = ${input.licenseId})`);
      return { success: true };
    }),
